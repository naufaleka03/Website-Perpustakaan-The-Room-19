...
export default function PaymentSummaryModal({ isOpen, onClose }) {
  ...
  const calculateAmount = () => {
    const basePrice = getStoredFormData().category === 'Student' ? 25000 : 35000;
    const groupPrice = getStoredFormData().members?.length ? basePrice * getStoredFormData().members.length : 0;
  ...
  const handlePaymentSuccess = async (result) => {
    try {
      ...
      const response = await submitSessionReservation(sessionData);

      if (response.success) {
        // Clear stored data after successful submission
        localStorage.removeItem('reservationFormData');
        window.location.href = `/user/dashboard/payment-finish?order_id=${result.order_id}&transaction_status=${result.transaction_status}`;
      } else {
      ...
      <div className="bg-white rounded-xl p-6 w-[400px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 mb-6">
          <div className="flex justify-between">
            ...
            <span className="text-sm font-medium text-[#666666]">{getStoredFormData().category}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-[#666666]">Date</span>
            <span className="text-sm font-medium text-[#666666]">{getStoredFormData().arrival_date}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-[#666666]">Shift</span>
            <span className="text-sm font-medium text-[#666666]">{getStoredFormData().shift_name}</span>
          </div>
          <div className="border-t border-gray-200 my-4"></div>
          <div className="flex justify-between">
            <span className="text-sm font-medium text-[#666666]">Total Amount</span>
            <span className="text-sm font-medium text-[#666666]">Rp {calculateAmount().toLocaleString()}</span>
          </div>
        </div>
