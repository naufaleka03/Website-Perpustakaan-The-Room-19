...
export default function DetailEventModal({ isOpen, onClose, event }) {
  if (!isOpen || !event) {
    return null;
  }
  ...
  const startTime = event.shift_start.substring(0, 5);
  const endTime = event.shift_end.substring(0, 5);
    ...
    >
      ...
      >
        ...
        <div className="space-y-1 border-t border-b py-2 text-xs font-['Poppins']">
          <Row label="Event Name" value={event.event_name} />
          <Row label="Event Date" value={formatDate(event.event_date)} />
          <Row label="Booking Date" value={formatDate(event.created_at)} />
          <Row label="Full Name" value={event.full_name} />
          <Row label="Shift" value={event.shift_name} />
          <Row label="Time" value={`${startTime} - ${endTime}`} />
          <Row
            label="Ticket Fee"
            value={`Rp${event.ticket_fee.toLocaleString("id-ID")}`}
          />
        </div>

        {event.group_members && event.group_members.length > 0 && (
